...
select cast(id_info as JSON), official_title, cast(study_design_info as JSON), cast(sponsors as JSON), overall_status, cast(enrollment as JSON), keyword, condition, cast(location as JSON), why_stopped, phase, study_type, number_of_arms, number_of_groups, condition_browse, intervention_browse, cast(primary_outcome as JSON), cast(secondary_outcome as JSON), cast(responsible_party as JSON), cast(eligibility as JSON) from hsdlc.ct_studies where overall_status != '' and overall_status in ('Completed', 'Approved for marketing', 'Withdrawn', 'Terminated', 'Suspended');
...
select cast(id_info as JSON), official_title, cast(study_design_info as JSON), cast(sponsors as JSON), overall_status, cast(enrollment as JSON), keyword, condition, cast(location as JSON), why_stopped, phase, study_type, number_of_arms, number_of_groups, condition_browse, intervention_browse, cast(primary_outcome as JSON), cast(secondary_outcome as JSON), cast(responsible_party as JSON), cast(eligibility as JSON) from hsdlc.ct_studies where overall_status != '' and overall_status in ('Completed', 'Approved for marketing', 'Withdrawn', 'Terminated', 'Suspended') and id_info.nct_id='NCT00074191'

"[""IRB00000929"",[""5729-1"",""ONC-99054-1"",""929"",""OHSU-5729-1""],""NCT00074191"",[]]",
"Combination Chemotherapy (Methotrexate, Procarbazine And CCNU), Intraventricular Cytarabine And Methotrexate, +/- Intra-Ocular Chemotherapy For Patients With Primary Central Nervous System Lymphoma","["""","""","""",""Treatment"","""","""",""None (Open Label)"",""""]","[[""OHSU Knight Cancer Institute"",""Other""],[[""National Cancer Institute (NCI)"",""NIH""]]]","Completed","[""Actual"",""1""]","[""intraocular lymphoma"",""primary central nervous system lymphoma""]","[""Lymphoma""]","[[[""Oregon Health & Science University Cancer Institute"",[""Portland"",""Oregon"",""97239-3098"",""United States""]],"""",["""","""","""","""","""","""",""""],["""","""","""","""","""","""",""""],[]]]","","Phase 2","Interventional","","","[""Lymphoma""]","[""Dexamethasone acetate"",""Dexamethasone"",""Methotrexate"",""Cytarabine"",""Lomustine"",""Procarbazine"",""BB 1101""]","[[""Survival as measured by clinical and radiographic response at 5 years following completion of study treatment"",""5 years"",""""]]","[[""Overall survival as measured by clinical and radiographic response"",""5 years"",""""],[""Progression-free survival as measured by clinical and radiographic response until tumor progression"",""5 years"",""""],[""Quality of Life (QOL) as measured by EORTC QOL before and after study treatment, every 6 months for 2 years, and then annually"",""5 years"",""""]]","[""Principal Investigator"",""OHSU Knight Cancer Institute"",""Edward Neuwelt"",""Professor""]","[[""""],"""",[""""],""All"","""",""16 Years"",""75 Years"",""No""]"
