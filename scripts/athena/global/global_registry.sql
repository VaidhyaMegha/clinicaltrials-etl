   SELECT   RTRIM(LTRIM(COALESCE(cast(to_base64url(md5(to_utf8(trials.id))) as varchar(8)),'') || '-'||
            COALESCE(cast(to_base64url(md5(to_utf8(ctri_number))) as varchar(8)),'')|| '-' ||COALESCE(cast(to_base64url(md5(to_utf8(nct_id))) as varchar(8)),''),'-'),'-')  ctd_id,
            ctri_number,         
            nct_id,         
            secondary_ids,        
            identifiers,
            ...
            CAST(MAP(ARRAY['ctris.scientific_title','cts.official_title','trials.scientific_title'],ARRAY[ctri_cts.scientific_title,ctri_cts.official_title,trials.scientific_title]) AS JSON)
            Scientific_title,
            ...
            CAST(MAP(ARRAY['ctris.secondary_outcome','cts.secondary_outcome','trials.secondary_outcomes'],
            ARRAY[ctri_cts.primary_outcome,'', trials.secondary_outcomes]) AS JSON)secondary_outcome,
            ...
            CAST(MAP(ARRAY['ctris.date_of_completion_global','ctri.date_of_completion_india','cts.completion_date','trials.completion_date'],ARRAY[ctri_cts.date_of_completion_global,ctri_cts.date_of_completion_india,ctri_cts.completion_date, trials.completion_date]) AS JSON) CompletionDate
   FROM (    
   SELECT ctri_number,
           id_info.nct_id nct_id,               
           secondary_ids,
           ctris.registered_on,
           cts.study_first_submitted first_received_date,
           ctris.type_of_study,
           cts.study_type, 
           ctris.public_title,
           cts.brief_title,
           ...
           ctris.target_sample_size,
           baseline.population baseline_population ,
           ctris.recruitment_status_global,
           ctris.recruitment_status_india,
           cts.overall_status,
           ctris.primary_outcome, 
           ctris.secondary_outcome,
           ctris.intervention_or_comparator_agent intervention,
           ...
           completion_date.text_node_value completion_date
  FROM global.ctri_studies ctris  
  FULL OUTER JOIN global.ct_studies cts ON
  substr(secondary_ids, strpos(secondary_ids, 'NCT'), 11)=id_info.nct_id  where ctri_number !='' or id_info.nct_id  != '' )  ctri_cts    FULL OUTER JOIN global.trials trials    ON substr(trials.identifiers, strpos(trials.identifiers,
  'NCT'), 11)=ctri_cts.nct_id
